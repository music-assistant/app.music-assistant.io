...
- Pure-Python DTLS 1.2 PSK implementation — no external C dependencies required

## Configuration

- Create an Entertainment Area in the Philips Hue app (Settings → Entertainment Areas) before adding the plugin
- In Music Assistant, go to Settings → Providers → Add Provider → `Hue Lights Sync`
- Enter the IP address of your Hue bridge, or let mDNS auto-discover it
...
- Join a Hue light player to any group that is currently playing — the lights will start reacting to the music
